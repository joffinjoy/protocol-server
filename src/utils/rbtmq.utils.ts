...
interface callbackType {
  (msg: AmqbLib.ConsumeMessage | null): void;
}
...
export class MQClient {
  ...
  public async connect(): Promise<void> {
    try {
      ...
      logger.info(
        `MQ Client Connected For: ${this.amqpUrl.split("@")[1].split(":")[0]}`
      );
    } catch (error) {
      throw new Exception(
        ExceptionType.MQ_ConnectionFailed,
        `MQ Client Connection Failed For URL: ${this.amqpUrl}`,
        500,
        error
      );
    }
  ...
  public async assertQueue(queue: string): Promise<void> {
    if (!this.channel) {
      throw new Exception(
        ExceptionType.MQ_ClientNotInitialized,
        "MQ Client is not connected",
        500
      );
    }
