...
export class ResponseCache {
  ...
  private constructor() {
    if (!getConfig().responseCache.enabled) {
      throw new Exception(
        ExceptionType.ResponseCache_NotEnabled,
  ...
  private async createExpireIndex() {
    ...
    try {
      const collection = this.dbClient
        .getDB()
        .collection(responseCacheCollection);
  ...
  public async cacheResponse(responseBody: any) {
    ...
    const ttlTime = moment
      .duration(responseBody.context.ttl ? responseBody.context.ttl : "PT0S")
      .asMilliseconds();
