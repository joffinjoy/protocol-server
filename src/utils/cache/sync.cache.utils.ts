...
import {
  SyncCacheDataType,
  syncCacheSchema,
} from "../../schemas/cache/sync.cache.schema";
...
export class SyncCache {
  ...
  private constructor() {
    if (getConfig().client.type != ClientConfigType.synchronous) {
      throw new Exception(
        ExceptionType.SyncCache_InvalidUse,
        "Sync Cache should be used only in case the client connection mode is synchronous.",
        400
      );
    }

    const syncCacheConfig = getConfig().client
      .connection as SyncrhonousClientConfigDataType;
  ...
  public async initCache(message_id: string, action: RequestActions) {
    ...
    const collection = this.dbClient
      .getDB()
      .collection(syncCacheCollectionName);
  ...
  public async getData(
    message_id: string,
    action: RequestActions
  ): Promise<SyncCacheDataType | null> {
    if (getConfig().client.type != ClientConfigType.synchronous) {
      throw new Exception(
        ExceptionType.SyncCache_InvalidUse,
        "Sync Cache should be used only in case the client connection mode is synchronous.",
