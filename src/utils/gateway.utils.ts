...
export class GatewayUtils {
    ...
    switch (getConfig().app.gateway.mode) {
      case GatewayMode.client: {
        switch (getConfig().app.mode) {
          case AppMode.bap: {
            await this.mqClient.consumeMessage(
              getConfig().app.gateway.inboxQueue,
              bapNetworkResponseSettler
            );
            break;
          }
          case AppMode.bpp: {
      ...
      case GatewayMode.network: {
        switch (getConfig().app.mode) {
          case AppMode.bap: {
            await this.mqClient.consumeMessage(
              getConfig().app.gateway.outboxQueue,
              bapClientTriggerSettler
            );
            break;
          }
          case AppMode.bpp:
  ...
  public async sendToClientSideGateway(data: any) {
    if (getConfig().app.gateway.mode === GatewayMode.client) {
      throw new Exception(
        ExceptionType.Gateway_InvalidUse,
        "Gateway is in client mode, cannot send data to client side gateway",
        500
      );
    }

    await this.mqClient.publishMessage(
      getConfig().app.gateway.inboxQueue,
      data
    );
  }
  ...
  public async sendToNetworkSideGateway(data: any) {
    if (getConfig().app.gateway.mode === GatewayMode.network) {
      throw new Exception(
        ExceptionType.Gateway_InvalidUse,
        "Gateway is in network mode, cannot send data to network side gateway",
        500
      );
    }
