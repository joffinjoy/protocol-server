...
const lookupCacheTTL = Moment.duration(
  process.env.lookupCacheTTL ? process.env.lookupCacheTTL : "PT10M"
).asSeconds();
...
export class LookupCache {
  ...
  private createQuery(parameters: LookupParameter): LookupParameter {
    return {
      ...
      subscriber_id: parameters.subscriber_id
        ? parameters.subscriber_id
        : emptyPlaceHolder,
      type: parameters.type ? parameters.type : emptyPlaceHolder,
      unique_key_id: parameters.unique_key_id
        ? parameters.unique_key_id
        : emptyPlaceHolder,
    };
  }

  public async cache(
    parameters: LookupParameter,
    subscribers: Array<SubscriberDetail>
  ): Promise<string> {
    const db: Db = getDb();
    const collection = db.collection(lookupCacheCollectionName);
  ...
  public async check(
    parameters: LookupParameter
  ): Promise<Array<SubscriberDetail> | null> {
    const db: Db = getDb();
    const collection = db.collection(lookupCacheCollectionName);
