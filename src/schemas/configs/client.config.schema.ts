...
export type SyncrhonousClientConfigDataType = z.infer<
  typeof syncrhonousClientConfigSchema
>;
export const parseSynchronousClientConfig = (
  config: any
): SyncrhonousClientConfigDataType => {
  if (!config) {
    throw new Exception(
      ExceptionType.Config_SynchronousClientConfig_NotFound,
      "Synchronous client config not found",
      404
    );
  }
  ...
  try {
    ...
    throw new Exception(
      ExceptionType.Config_SynchronousClientConfig_Invalid,
      "Invalid synchronous client config",
      400,
      e
    );
  }
...
export const parseWebhookClientConfig = (
  config: any
): WebhookClientConfigDataType => {
  if (!config) {
    throw new Exception(
      ExceptionType.Config_WebhookClientConfig_NotFound,
      "Webhook client config not found",
      404
    );
  }
  ...
  try {
    ...
    throw new Exception(
      ExceptionType.Config_WebhookClientConfig_Invalid,
      "Invalid webhook client config",
      400,
      e
    );
  }
...
export type MessageQueueClientConfigDataType = z.infer<
  typeof messageQueueClientSchema
>;

export const parseMessageQueueClientConfig = (
  config: any
): MessageQueueClientConfigDataType => {
  if (!config) {
    throw new Exception(
      ExceptionType.Config_MessageQueueClientConfig_NotFound,
      "Message queue client config not found",
      404
    );
  }
  ...
  try {
    ...
    throw new Exception(
      ExceptionType.Config_MessageQueueClientConfig_Invalid,
      "Invalid message queue client config",
      400,
      e
    );
  }
...
export const parseClientConfig = (config: any): ClientConfigDataType => {
  if (!config) {
    throw new Exception(
      ExceptionType.Config_ClientConfig_NotFound,
  ...
  if (Object.keys(config).length === 0) {
    throw new Exception(
      ...
      "Not even one type of Client Configuration is found.",
      400
    );
  }
