...
export const parseGatewayAppConfig = (
  config: any
): GatewayAppConfigDataType => {
  if (!config) {
    throw new Exception(
      ExceptionType.Config_GatewayAppConfig_NotFound,
      "Gateway app config not found",
      404
    );
  }
  ...
  try {
    ...
    throw new Exception(
      ExceptionType.Config_GatewayAppConfig_Invalid,
      "Invalid gateway app config",
      400,
      e
    );
  }
